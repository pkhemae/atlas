...
- API access: typed Tuyau client (`createTuyau` + `@atlas/api/registry`); response types flow from the API's controllers/transformers — controllers must use the branded response helpers (`response.badRequest(...)`, `response.unprocessableEntity(...)`, `serialize(...)`) or Tuyau's inference degrades to `unknown` and forces client-side casts. The base URL comes from `VITE_API_URL`. Tuyau auto-switches to multipart when the body contains a `File`; `null` serializes as an empty field, which the API reads as "remove" for images.
...
- Themes: named `[data-theme]` token blocks in `apps/desktop/src/styles.css` ONLY (never in `packages/ui` — marketing depends on its `.dark` block). Registry in `src/theme/index.ts` (localStorage + `theme:changed` dock event, same pattern as i18n). Adding a theme = one full CSS block (every token of the atlas block incl. `--nav-active-bar`, `--activity-1..4`, `--chart-1`, `--tier-*` + `--tier-*-glow` (levels-road metals — daylight uses deeper variants for legibility), `--radius`, `color-scheme`) + a `THEMES` entry + one i18n key. No hardcoded accent colors in components — use tokens; the only exceptions are the commented "deliberately dark" zones (tooltip pills, dock pill, banner Import overlay, dialog scrim — they stay dark in light themes too). Light themes are supported: a theme declares `dark: boolean` (+ a native `background` RGB) in `THEME_META` and the registry toggles the `dark` class; form fills are token-driven (`bg-input/30`, never `dark:bg-input/30`); floating-surface elevation uses the shared `--surface-ring`/`--surface-shadow` tokens (defined in `packages/ui` globals for both schemes — never hardcode a hairline ring color).
- i18n: i18next with TypeScript-bundled catalogs in `src/i18n/locales/` — `en.ts` (`as const`) is the source of truth and types every `t()` key; `fr.ts` is `satisfies`-checked, so a missing translation is a compile error (any change to `en.ts` must touch `fr.ts` in the same commit). All user-facing copy goes through `t()` — `ui/` components may call `useTranslation` directly (translation is presentation), but `@atlas/ui` never imports i18next (label props with English defaults instead, see `password-input.tsx`). Locale-sensitive formatting goes through `currentLocale()`/the `duration.*` keys — never hardcode `"en-US"` or glue unit suffixes. The dock webview follows along via localStorage at boot and the `i18n:changed` Tauri event live; beware module-scope constants and memos that bake localized strings (depend on `i18n.language`).
